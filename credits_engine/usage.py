...
def get_usage(model_name, input_tokens, output_tokens, image_count=0):
    ...
    is_image_model = image_count > 0 or model_name in IMAGE_MODELS
    ...
    image_cost = 0.0
    if image_count > 0:
        per_image = market_model_price.get("per_image", 0.03)
        image_cost = float(per_image) * int(image_count)
...
def get_total_usage(usage: list[dict], no_of_images: int = 0) -> dict:
    ...
    if no_of_images and not any(pm.get("image_count") for pm in per_model):
        img_res = get_usage("google/gemini-2.5-flash-image", 0, 0, image_count=no_of_images)
        if "error" not in img_res:
            img_credits = Decimal(str(img_res["credits"]))
            total += img_credits
            per_model.append(
                {
                    "model": "google/gemini-2.5-flash-image",
