...
End-to-end quality benchmarking of the **CEO agent** lives in `evals/e2e/` (`run_ceo_e2e.py` harness, `run_judging.py` judging). Latest run: **27 runs / 81 judge verdicts**. Full write-up: [`eval_report.md`](eval_report.md).
...
|---|---|
| ![Scores by tweak and mode](answera_scores_eda.png) | ![Runtime and tokens](eda.png) |
...
- Required env vars:
  - Frontend (`NEXT_PUBLIC_*`, inlined at build time — local dev reads them from the **repo-root `.env`** via `frontend/scripts/run-next.js`, no separate `frontend/.env.local` needed; **production sets them in the Vercel project environment**): `NEXT_PUBLIC_API_URL` (public EC2 backend URL), `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, optional `NEXT_PUBLIC_AUTH_REDIRECT_URL`, plus `NEXT_PUBLIC_RAZORPAY_KEY_ID` for checkout
  - ⚠️ The frontend is deployed separately on Vercel (https://get-cofounder.tech) — it is **not** part of `docker-compose.yaml` (backend-only stack). `NEXT_PUBLIC_*` values are frozen into the client bundle at build time, so changing one requires a Vercel redeploy. (`frontend/Dockerfile` remains only as a reference for a standalone container; compose no longer builds or runs it.)
...
2. Browser opens `https://checkout.razorpay.com/v1/checkout.js` (injected once via `loadRazorpayCheckoutScript()` in `frontend/src/app/(app)/billing/page.tsx:68`) — keyed only by public `NEXT_PUBLIC_RAZORPAY_KEY_ID`.
...
- `RAZORPAY_KEY_SECRET` **never leaves the backend** (`backend/api/payments.py:15`); env vars are `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET` (backend) and `NEXT_PUBLIC_RAZORPAY_KEY_ID` (frontend).
