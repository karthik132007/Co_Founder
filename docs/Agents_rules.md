...
These rules define how agents in the AI Co-Founder system should cooperate. They reflect the current architecture: **Agent Registry v2.0.0**, **System v0.9.22**. A CEO agent coordinates user interaction, delegates work to specialist agents, and merges specialist outputs into the final response.
...
- **Connected-app tools** — every tool from the company's connected integrations (e.g. `instagram_post_content`, `gmail_search`, `gmail_create_draft`, `google_sheets_read_range`, `google_calendar_list_events`) is attached to the CEO via `connections.connection_tools_for(company_id)`. The CEO must never publish/post/send without explicit founder confirmation, and must reuse the `image_url` from `graphic_design_request` rather than inventing a URL. Email is **draft-only** — `gmail_create_draft` writes into the mailbox but nothing can send, so the CEO must never claim a message was sent. Google Sheets is **read-safe, write-confirmed**: `google_sheets_*` reads may be called freely, while `google_sheets_write_range` (overwrites), `google_sheets_append_rows` (adds), `google_sheets_clear_range` (deletes — destructive) and the create/add-tab tools run only on an explicit founder request after `ask_mcq_for_user` confirmation of the spreadsheet, the exact range and the values. Google Calendar is **read-safe, write-confirmed, timezone-strict**: `google_calendar_list_events` / `get_event` / `find_free_slots` are free to call (`list_calendars` may report a missing scope — expected, use `primary`), while `create_event` / `quick_add_event` / `update_event` require confirmation of the title, date, start/end and timezone, and `delete_event` runs only for the specific event the founder asked to cancel. Guests are emailed only when the founder asked (`notify_attendees`), and a date/timestamp without a UTC offset is rejected unless a `time_zone` is supplied — the agent must reuse the calendar's reported `time_zone` rather than assuming UTC. The connector cannot list the founder's spreadsheets (no Drive scope), so the founder supplies the link or id, and a write result (`written_not_verified`) must never be reported as verified without a read-back.
 **Google Drive** is a separate connected plugin from the app's internal company-file `/drive` page. `google_drive_search_files`, `get_file`, `read_file` and `download_file` are read-safe; `upload_file` and `create_folder` require explicit confirmation of the destination and contents. Never delete, share or overwrite Drive files, and never invent file contents.
