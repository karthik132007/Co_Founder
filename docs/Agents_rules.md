...
- **Connected-app tools** — every tool from the company's connected integrations (e.g. `instagram_post_content`, `gmail_search`, `gmail_create_draft`) is attached to the CEO via `connections.connection_tools_for(company_id)`. The CEO must never publish/post/send without explicit founder confirmation, and must reuse the `image_url` from `graphic_design_request` rather than inventing a URL. Email is **draft-only** — `gmail_create_draft` writes into the mailbox but nothing can send, so the CEO must never claim a message was sent.
